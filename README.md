#  McDonald's South America Sales Analysis

_Analyzing the sales performance, profitability, and customer satisfaction trends to support smarter business decisions using Microsoft Excel._

---

## Table of Contents
- [Overview](#overview)
- [Project Objective](#project-objective)
- [Dataset](#dataset)
- [Tools & Technologies](#tools--technologies)
- [Dashboard Features](#dashboard-features)
- [Key Insights](#key-insights)
- [Dashboard](#dashboard)
- [Conclusion](#conclusion)
- [Role & Contribution](#role--contribution)
- [Author](#author)

---

## Overview
This project focuses on analyzing the McDonald's South America sales performance to understand revenue trends, profit margins, and customer satisfaction metrics. An interactive Excel dashboard was developed to transform the raw business data into clear, actionable insights for better decision making.

---

## Project Objective
The primary goals of this project are:
- Analyze the overall sales performance across South America  
- Track the profit and revenue trends  
- Evaluate customer satisfaction metrics  
- Identify monthly sales trends  
- Provide data driven insights to improve business performance  

---

## Dataset
🗂️ <a href="https://github.com/SHAKAR-AFZAL/mcdonalds-sales-analytics-dashboard/blob/main/Mcdonalds-Sales-Analytics.xlsx">Dataset View</a>  
 
The McDonald's South America dataset is stored in Excel format and includes the following attributes:
- Country  
- Sales Revenue  
- Profit  
- Customer Satisfaction  
- Monthly Sales Data  
- Product Performance  
- Regional Performance  

---

## Tools & Technologies
- **Microsoft Excel**
- Pivot Tables  
- Pivot Charts  
- Slicers  
- Filters  
- Data Cleaning  
- Conditional Formatting  
- Dashboard Design    

---

## Dashboard Features

**Interactive Slicers**
- Country  
- Year  
- Product Category  

**Key Visuals**
- Monthly Sales Trend (2021–2022)  
- Customer Satisfaction Radar Chart  
- Country wise Sales Map  
- Sales KPI Cards  

---

## Key Insights

### Sales Performance
- Sales showed steady growth toward the end of 2022  
- Mid year performance indicated the slight seasonal decline  
- Strong recovery observed in final months  

### Profit Analysis
- Profit trends closely followed sales growth  
- High revenue months generated stronger profit margins  
- Some regions showed opportunities for cost optimization  

### Customer Satisfaction Trends
- High overall customer satisfaction scores  
- Hygiene and quality received strong ratings  
- Service and speed showed improvement opportunities  

### Regional Performance
- Country wise comparison revealed top performing markets  
- Some regions demonstrated consistent growth  
- Geographic insights highlighted expansion opportunities  

---

## Dashboard
<a href="https://github.com/SHAKAR-AFZAL/mcdonalds-sales-analytics-dashboard/blob/main/Dashboard%20Picture.png">📊 Dashboard Picture</a>

The Excel dashboard displays:
- Sales performance overview  
- Profit Trends  
- Customer satisfaction metrics  
- Country-wise performance  
- Monthly sales trends  


<img width="1919" height="797" alt="Dashboard Picture" src="https://github.com/user-attachments/assets/a886d074-af02-4bd7-a1d0-0df193745818" />

---

## Conclusion
The analysis shows that **sales performance, profit margins, and customer satisfaction** are key drivers of business success. Strong performance in certain regions highlights growth opportunities, while satisfaction metrics reveal areas for operational improvements.

Businesses can use these insights to:
- Focus on high performing regions  
- Improve customer satisfaction  
- Optimize operational performance  
- Make data driven strategic decisions  

---

## Role & Contribution
- Data Analysis  
- Data Preparation  
- Data Cleaning  
- Pivot Table  
- Chart Creation  
- Interactive Dashboard Design  
- Business Insight Generation  
- Data Driven Insights for Business Growth  

---

## Author
**Mian Shakar Afzal**  

[LinkedIn](https://www.linkedin.com/in/mian-shakar-afzal-959b443a8/) | [GitHub](https://github.com/SHAKAR-AFZAL) | [Email](mailto:mianshakarafzal@gmail.com)
